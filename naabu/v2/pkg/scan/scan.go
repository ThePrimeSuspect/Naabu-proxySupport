...
func (s *Scanner) GetRotateProxyDialer() proxy.Dialer {
	if len(s.proxyDialerList) == 0 {
		return nil
	}
	...
	gologger.Info().Msgf("rotate index: %d\n", s.rotateIndex)

	currentProxyDialer := s.proxyDialerList[s.rotateIndex]
	s.rotateIndex += 1
	if s.rotateIndex >= len(s.proxyDialerList) {
		s.rotateIndex = 0
	}

	return currentProxyDialer
}
...
func NewScanner(options *Options) (*Scanner, error) {
	...
	if strings.Contains(options.Proxy, ",") && strings.Contains(options.ProxyAuth, ",") {
		proxyAuthList := strings.Split(options.ProxyAuth, ",")
		proxyList := strings.Split(options.Proxy, ",")
		if len(proxyAuthList) != len(proxyList) {
			gologger.Info().Msgf("proxyAuth and proxy should have same length")
			return nil, fmt.Errorf("proxy and proxyAuth should have same length")
		}
		for i, proxyValue := range proxyList {
			var auth *proxy.Auth = nil
			if proxyValue == "" {
				gologger.Info().Msgf("empty proxy")
				continue
			}

			proxyAuthValue := proxyAuthList[i]
			if proxyAuthValue != "-" && strings.Contains(proxyAuthValue, ":") {
				credentials := strings.SplitN(proxyAuthValue, ":", 2)
			...
			if proxyValue != "" {
				proxyDialer, err := proxy.SOCKS5("tcp", proxyValue, auth, &net.Dialer{Timeout: options.Timeout})
				if err != nil {
					return nil, err
				}
				//scanner.proxyDialer = proxyDialer
				scanner.proxyDialerList = append(scanner.proxyDialerList, proxyDialer)
			}
		}
		if len(scanner.proxyDialerList) > 0 {
			scanner.proxyDialer = scanner.proxyDialerList[0]
			gologger.Info().Msgf("all proxyDialer is : %+v\n", scanner.proxyDialerList)
		}
...
func (s *Scanner) ConnectPort(host string, p *port.Port, timeout time.Duration) (bool, error) {
	...
	currentProxyDialer := s.GetRotateProxyDialer()
	if currentProxyDialer != nil {
		s.proxyDialer = currentProxyDialer
		gologger.Info().Msgf("connect use proxyDialer: %+v\n", s.proxyDialer)
	}
